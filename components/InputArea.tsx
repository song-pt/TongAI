...
import { ArrowUp, Loader2, GraduationCap, BookOpenText } from 'lucide-react';
import { translations } from '../utils/translations';
import { Language, AiMode } from '../types';

interface InputAreaProps {
  onSend: (message: string, grade: string, subject: string) => void;
  isLoading: boolean;
  onSubjectChange?: (subject: string) => void;
  language: Language;
  aiMode?: AiMode; // Add optional AI Mode prop
}

const InputArea: React.FC<InputAreaProps> = ({ onSend, isLoading, onSubjectChange, language, aiMode = 'solver' }) => {
  ...
  // If switched to normal mode, maybe reset subject/grade, but keeping state is fine
  const isSolverMode = aiMode === 'solver';

  const handleSubmit = (e?: React.FormEvent) => {
    e?.preventDefault();
    if (input.trim() && !isLoading) {
      onSend(input, grade, subject);
      setInput('');
      // Keep grade and subject selected for convenience
  ...
  const getPlaceholder = (sub: string) => {
    if (!isSolverMode) return t.placeholderDefault; // Generic placeholder for normal mode
  ...
  return (
    <div className="w-full max-w-3xl mx-auto transition-all duration-300 ease-in-out">
      <form 
        ...
        className={`
          relative flex items-end gap-2 p-2 bg-white rounded-2xl border transition-all duration-300
